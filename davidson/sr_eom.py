...
class SR_EOM:
    def __init__(self,psi4_wfn, unitary, sym=0, verbose=True, screen_thresh_H=1e-9, screen_thresh_exp=1e-9, maxk=19):
        ...
        self.forte_objs = forte.utils.prepare_forte_objects(psi4_wfn, mo_spaces={'GAS1':list(psi4_wfn.doccpi().to_tuple()), 'GAS3':list((psi4_wfn.nmopi() - psi4_wfn.doccpi()).to_tuple())})
        ...
        if (self.verbose): print(f'{self.occ_sym=}')
        ...
        for h in range(self.nirrep):
            for i in range(self.naelpi[h]): self.hfref.set_alfa_bit(irrep_start[h] + i, True)
            for i in range(self.nbelpi[h]): self.hfref.set_beta_bit(irrep_start[h] + i, True)        
    ...
    def make_ee_eom_basis(self, max_exc):
        _ee_eom_basis = [forte.StateVector({self.hfref:1.0})] # Reference determinant (0 excitations)
    ...
    def make_ip_eom_basis(self, max_exc):
        ...
        _ip_eom_basis = [forte.StateVector({self.hfref:1.0})] # Reference determinant (0 excitations)
                            ...
                            for bo in itertools.combinations(self.occ, self.nbel-bk):
                                ...
                                for bv in itertools.combinations(self.vir, bj):
                                    ...
                                    if (ao_sym ^ av_sym ^ bo_sym ^ bv_sym == self.sym):
                                        ...
                                        for i in bo: d.set_beta_bit(i, True)
                                        for i in bv: d.set_beta_bit(i, True)
                                        _ip_eom_basis.append(forte.StateVector({d:1.0}))
    ...
    def make_ea_eom_basis(self, max_exc):
        ...
        _ea_eom_basis = [forte.StateVector({self.hfref:1.0})] # Reference determinant (0 excitations)
        max_exc = 2
                            ...
                            for bo in itertools.combinations(self.occ, self.nbel-bj):
                                ...
                                for bv in itertools.combinations(self.vir, bk):
                                    ...
                                    if (ao_sym ^ av_sym ^ bo_sym ^ bv_sym == self.sym):
                                        ...
                                        for i in bo: d.set_beta_bit(i, True)
                                        for i in bv: d.set_beta_bit(i, True)
                                        _ea_eom_basis.append(forte.StateVector({d:1.0}))
    ...
    def run_eom(self, max_exc, mode, print_eigvals=True):
        ...
        if (self.unitary):
            ...
            self.eom_eigval, self.eom_eigvec = davidson.kernal(n_lowest=3)
            ...
            for i in range(1,len(self.eom_eigval)):
                ...
                s /= 4
                print(f'{i:^4d} {self.eom_eigval[i]:^25.12f} {abs(s2_val):^10.3f} {abs(s):^5.1f}')
            print(f"{'='*4:^4}={'='*25:^25}={'='*10:^10}={'='*5:^5}")  
        else:
...
if __name__ == '__main__':
    ...
    if (test == 1):
        ...
        escf, psi4_wfn = forte.utils.psi4_scf(geometry,basis='sto-6g',reference='rhf',options={'E_CONVERGENCE' : 1.e-12})
        ...
        ccsd_unitary = SR_EOM(psi4_wfn,unitary=True,screen_thresh_H=1e-8, screen_thresh_exp=1e-8,maxk=8)
        ccsd_unitary.make_T(max_exc=3)
        ccsd_unitary.run_ccn()
        ccsd_unitary.make_ee_eom_basis(3)
        hbar_ee_ccsd_unitary = ccsd_unitary.make_hbar(ccsd_unitary.ee_eom_basis, algo='oprod')
        ...
        symmetry c2v
        """
        escf, psi4_wfn = forte.utils.psi4_scf(geometry,basis='cc-pvdz',reference='rhf',options={'E_CONVERGENCE' : 1.e-12})
