...
def cc_variational_functional(
    t, op, ref, ham_op, exp_op, screen_thresh_H, screen_thresh_exp, maxk
):
    """
    ...
    # Step 1. Compute exp(S)|Phi>
    wfn = exp_op.compute(
        op, ref, scaling_factor=1.0, maxk=maxk, screen_thresh=screen_thresh_exp
    )
    ...
    # Step 3. Compute exp(-S) H exp(S)|Phi>
    R = exp_op.compute(
        op, Hwfn, scaling_factor=-1.0, maxk=maxk, screen_thresh=screen_thresh_exp
    )

    # Step 4. Get the energy: <Phi|R>
    # E = <ref|R>, R is a SparseState, which can be looked up by the determinant
    energy = forte.overlap(ref, R).real
...
class EOM_MRCC:
    ...
    def get_casci_wfn(self, nelecas):
        ...
        if False:
                ...
                for bstr in itertools.combinations(aorbs_rel_fci, nact_bel_fci):
                    ...
                    if asym ^ bsym == self.sym:
                        ...
                        for i in corbs_fci:
                            d.set_beta_bit(i, True)
                        for i in astr:
                            d.set_alfa_bit(aorbs_fci[i], True)
                        for i in bstr:
                            d.set_beta_bit(aorbs_fci[i], True)
            ...
            for i in range(len(self.dets_fci)):
                for j in range(i + 1):
                    self.H_fci[i, j] = as_ints_fci.slater_rules(
                        self.dets_fci[i], self.dets_fci[j]
                    )
                    if i == j:
                        self.H_fci[i, j] += (
                            as_ints_fci.scalar_energy()
                            + as_ints_fci.nuclear_repulsion_energy()
                        )
    ...
    def CAS_INT(self, nelecas, internal_max_exc):
                        ...
                        for bo in itertools.combinations(self.act, nb):
                            ...
                            for bv in itertools.combinations(self.act, nb):
                                ...
                                if ao_sym ^ av_sym ^ bo_sym ^ bv_sym == self.sym:
                                    T_op_temp = forte.SparseOperator(
                                        antihermitian=False
                                    )
                                    ...
                                    T_op_temp.add_term(l, 1.0, allow_reordering=False)
                                    self.cas_int_basis.append(
                                        forte.apply_operator(T_op_temp, self.psi)
                                    )
                                    ...
                                    if n == 1:
                                        ...
                                        diag_2.append(
                                            -self.gamma2[
                                                r2_idx_4, r2_idx_3, r2_idx_1, r2_idx_2
                                            ]
                                            - 2
        ...
        def _act_spin_idx(s):
            return _act_idx[s[1]] * 2 if s[0] else _act_idx[s[1]] * 2 + 1

        _P_full += np.einsum(
            "ik,ab,jc->ibcajk", _I_act, _I_act, self.gamma1, optimize="optimal"
        )
        _P_full -= np.einsum(
            "ik,ac,jb->ibcajk", _I_act, _I_act, self.gamma1, optimize="optimal"
        )
        _P_full += np.einsum(
            "ij,ac,kb->ibcajk", _I_act, _I_act, self.gamma1, optimize="optimal"
        )
        _P_full -= np.einsum(
            "ij,ab,kc->ibcajk", _I_act, _I_act, self.gamma1, optimize="optimal"
        )
        ...
        for i in range(len(self.cas_int_basis)):
            for j in range(i + 1):
                ...
                cas_int_H[j, i] = forte.overlap(self.cas_int_basis[j], Hwfn).real
        ...
        for i in range(len(self.cas_int_basis)):
            for j in range(i + 1):
                S[i, j] = forte.overlap(
                    self.cas_int_basis[i], self.cas_int_basis[j]
                ).real
                S[j, i] = S[i, j]
        ...
        S_squareroot = np.diag(1.0 / np.sqrt(eigval[eigval > 1e-9]))
    ...
    def Harper_test(self, nelecas):
        ...
        a1_energy = (
            self.as_ints.slater_rules(a1_det[0], a1_det[0])
            + self.as_ints.scalar_energy()
            + self.as_ints.nuclear_repulsion_energy()
        )
        b2_energy = (
            self.as_ints.slater_rules(b2_det[0], b2_det[0])
            + self.as_ints.scalar_energy()
            + self.as_ints.nuclear_repulsion_energy()
        )
...
if __name__ == "__main__":
    ...
    # FCI 1 singlet energy: -1.14007348080257
    # 2.0
    # FCI 1 triplet energy: -0.8768102809804321
    # 2.9582283945787943e-30
    # FCI 2 singlet energy: -0.702809556769195
    # 2.000000000000001
    # FCI 2 triplet energy: -0.4882296865807753
    # 1.6613514297885678e-28
    elif test == 7:
        psi4.core.set_output_file("h2.dat", False)
        mol = psi4.geometry(
            """
        ...
        # CASSCF    -1.1235249497297586   -0.9964666722636089 0.12705827746
        # no He    -1.126778351084 1.0   -0.996466672374 5.0 0.13031167871
        # With He -3.991030269511 1.0   -3.862656336176 5.0 0.12837393333
    elif test == 8:
